...
#define LTE_POW_PAD IMX_GPIO_NR(2, 11)
#define LTE_RST_PAD IMX_GPIO_NR(1, 5)

#define BTWIFI_POW_PAD IMX_GPIO_NR(1, 15)
#define BTWIFI_VBAT_PAD IMX_GPIO_NR(4, 0)

#define WL_REGON_PAD IMX_GPIO_NR(2, 10)
#define WL_WAKEHOST_PAD IMX_GPIO_NR(2, 9)

#define M2_POWER_PAD IMX_GPIO_NR(3, 25)
#define M2_RESET_PAD IMX_GPIO_NR(4, 21)
#define M2_WAKE_PAD IMX_GPIO_NR(2, 20)


static iomux_v3_cfg_t const let_power_pads[] = {
	IMX8MM_PAD_SD1_STROBE_GPIO2_IO11 | MUX_PAD_CTRL(NO_PAD_CTRL),
	IMX8MM_PAD_GPIO1_IO05_GPIO1_IO5 | MUX_PAD_CTRL(NO_PAD_CTRL),
};

static iomux_v3_cfg_t const ap6212_power_pads[] = {
...
static iomux_v3_cfg_t const m2_power_pads[] = {
	IMX8MM_PAD_SAI5_MCLK_GPIO3_IO25 | MUX_PAD_CTRL(NO_PAD_CTRL),
	IMX8MM_PAD_SAI2_RXFS_GPIO4_IO21 | MUX_PAD_CTRL(NO_PAD_CTRL),
	IMX8MM_PAD_SD2_WP_GPIO2_IO20 | MUX_PAD_CTRL(NO_PAD_CTRL),

};


static void myd_init(void)
{
	imx_iomux_v3_setup_multiple_pads(let_power_pads,ARRAY_SIZE(let_power_pads));
	imx_iomux_v3_setup_multiple_pads(ap6212_power_pads,ARRAY_SIZE(ap6212_power_pads));
	imx_iomux_v3_setup_multiple_pads(m2_power_pads,ARRAY_SIZE(m2_power_pads));
	
    /*LET power */    
	gpio_request(LTE_POW_PAD, "LTE_POW_PAD");
	gpio_direction_output(LTE_POW_PAD, 1);
    /*LET rst */    
	gpio_request(LTE_RST_PAD, "LTE_RST_PAD");
	gpio_direction_output(LTE_RST_PAD, 0);

	/*wifi and bt power */    
	gpio_request(BTWIFI_POW_PAD, "BTWIFI_POW_PAD");
	gpio_direction_output(BTWIFI_POW_PAD, 1);

	/*wifi and bt vbat */    
	gpio_request(BTWIFI_VBAT_PAD, "BTWIFI_VBAT_PAD");
	gpio_direction_output(BTWIFI_VBAT_PAD, 0);

	/*wifi REG ON  */    
	gpio_request(WL_REGON_PAD, "WL_REGON_PAD");
	gpio_direction_output(WL_REGON_PAD, 1);
	gpio_request(WL_WAKEHOST_PAD, "WL_WAKEHOST_PAD");
	gpio_direction_output(WL_WAKEHOST_PAD, 1);

	/* M.2 PIN */
	gpio_request(M2_POWER_PAD, "M2_POWER_PAD");
	gpio_direction_output(M2_POWER_PAD, 1);
	gpio_request(M2_RESET_PAD, "M2_RESET_PAD");
	gpio_direction_output(M2_RESET_PAD, 0);
	mdelay(5);
	gpio_direction_output(M2_RESET_PAD, 1);
	gpio_request(M2_WAKE_PAD, "M2_WAKE_PAD");
	gpio_direction_output(M2_WAKE_PAD, 0);
	return ;
}
